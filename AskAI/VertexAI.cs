...
namespace AskAI
{
    public static class VertexAI
    {
        ...
        public static async Task<(string, string, string)> AskAsync(string prompt, AskAIConfig config, string apiKey, string modelId)
        {
            ...
            return await SendRequestAsync(apiUrl, requestBody);
        }

        public static async Task<string> RefineResponseAsync(string rawResponse, AskAIConfig config, string apiKey)
        {
            var modelId = config.SmartRefine.Model;
            var apiUrl = $"{config.ApiUrl}{modelId}:generateContent?key={apiKey}";
            ...
            var requestBody = new VertexAIRequest
            {
                SystemInstruction = new Instruction
                {
                    Parts = new List<Part> { new Part { Text = config.SmartRefineSystemPrompt } }
                },
                Contents = new List<Content>
                {
                    new Content
                ...
                GenerationConfig = new GenerationConfig
                {
                    Temperature = config.SmartRefine.Temperature,
                    TopP = config.SmartRefine.TopP,
                    MaxOutputTokens = 8192
                }
            };

            var (refinedText, _, _) = await SendRequestAsync(apiUrl, requestBody);
            return string.IsNullOrEmpty(refinedText) ? rawResponse : refinedText;
        }

        private static async Task<(string responseText, string requestBody, string responseBody)> SendRequestAsync(string url, VertexAIRequest request)
        {
